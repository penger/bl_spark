...
${PREFIX}check_if_not_zero_exit  ssh oracle@{NEW_DB_IP} "/home/oracle/customize_clearoracletb.sh m_uec_jr_all_jr cdate ${task_date} "
#数据从hive导入到oracle
...
${PREFIX}check_if_not_zero_exit  sqoop export --connect jdbc:oracle:thin:@{NEW_DB_IP}:1521:report --username idmdata --password bigdata915 --table IDMDATA.M_UEC_JR_ALL_JR  --export-dir /user/hive/warehouse/idmdata.db/m_uec_jr_all_jr/dt=${task_date}/  --fields-terminated-by '\t' --input-null-string '\\N' --input-null-non-string '\\N' --m 1
